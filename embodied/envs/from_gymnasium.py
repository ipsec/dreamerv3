...
class FromGymnasium(embodied.Env):
  ...
  def obs_space(self):
    ...
    spaces = {k: self._convert(v, unbounded=True) for k, v in spaces.items()}
    return {
        **spaces,
  ...
  def _convert(self, space, unbounded=False):
    if hasattr(space, 'n'):
      return elements.Space(np.int32, (), 0, space.n)
    if unbounded and np.issubdtype(space.dtype, np.floating):
      return elements.Space(space.dtype, space.shape, -np.inf, np.inf)
